...
void CustomAllReduce(DLTensor* send, int strategy, DLTensor* recv) {
  ...
  IPCMemoryContext ipc_ctx = ipc_alloc->GetIPCMemoryContext(send->data);
  params.barrier_flag = ipc_ctx.barrier_flag++;
  for (int i = 0; i < ctx->worker->num_workers; ++i) {
    params.peer_comm_buffer_ptrs[i] = ipc_ctx.remote_data[i];
  }
  for (int i = 0; i < ctx->worker->num_workers; ++i){
      params.peer_barrier_ptrs_in[i] = reinterpret_cast<uint32_t*>(ipc_ctx.barrier_in[i]);
  }
  for (int i = 0; i < ctx->worker->num_workers; ++i){
      params.peer_barrier_ptrs_out[i] = reinterpret_cast<uint32_t*>(ipc_ctx.barrier_out[i]);
  }

  customAllReduceInternal(params, recv->data, TensorSize(send), send->dtype.bits / 8, send->dtype, static_cast<AllReduceStrategyType>(strategy), ctx->default_stream);
}
