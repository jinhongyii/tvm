 ...
 public:
  ...
  Stmt VisitStmt_(const AttrStmtNode* op) final {
    if (const IterVarNode* iter_var = op->node.as<IterVarNode>()) {
      auto min = VisitExpr(iter_var->dom->min);
      auto extent = VisitExpr(iter_var->dom->extent);
      ...
      if (min.same_as(iter_var->dom->min) && extent.same_as(iter_var->dom->extent) &&
          var.same_as(iter_var->var)) {
        new_iter_var = GetRef<IterVar>(iter_var);
      } else {
        new_iter_var = IterVar(Range::FromMinExtent(min, extent), var, iter_var->iter_type,
