...
*/
class BufferFlattenerForFeature : public StmtExprMutator {
public:
 static Stmt Flatten(const PrimFunc& f) { return BufferFlattenerForFeature().VisitStmt(f->body); }
...
private:
 ...
 static Stmt MakeAllocStmt(const Buffer& buffer, Stmt body) {
   ...
   body = BufferRealize(buffer, bounds, const_true(), std::move(body));
   return body;
 }
...
PrimFunc FlattenBufferForFeature(PrimFunc f) {
 // Only apply this pass to TIR that is not from TE schedules
 if (!IsFromLegacyTESchedule(f)) {
   PrimFuncNode* fptr = f.CopyOnWrite();
   fptr->body = BufferFlattenerForFeature::Flatten(f);
   return f;
 } else {
   return f;
 }
...
Pass FlattenBufferForFeature() {
 auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
   return FlattenBufferForFeature(std::move(f));
 };
 return CreatePrimFuncPass(pass_func, 0, "tir.FlattenBufferForFeature", {});
}
