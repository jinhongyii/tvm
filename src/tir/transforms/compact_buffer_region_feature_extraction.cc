 ...
 */
class BufferAccessRegionCollectorFeatureExtraction : public StmtExprVisitor {
 public:
  static std::unordered_map<Buffer, Region, ObjectPtrHash, ObjectPtrEqual> Collect(
      const PrimFunc& f) {
    BufferAccessRegionCollectorFeatureExtraction collector;
    collector(f->body);
 ...
 private:
  ...
  BufferAccessRegionCollectorFeatureExtraction() = default;
...
class StorageAlignCollectorFeatureExtraction : public StmtVisitor {
 public:
  static std::unordered_map<Buffer, StorageAlignAnnotation, ObjectPtrHash, ObjectPtrEqual> Collect(
      const PrimFunc& f) {
    StorageAlignCollectorFeatureExtraction collector;
    collector(f->body);
...
class BufferCompactorFeatureExtraction : public StmtExprMutator {
 public:
  static Stmt Compact(
      const PrimFunc& f,
    ...
    BufferCompactorFeatureExtraction compactor(std::move(buffer_info));
 ...
 private:
  ...
  explicit BufferCompactorFeatureExtraction(
      std::unordered_map<Buffer, BufferAllocInfo, ObjectPtrHash, ObjectPtrEqual> buffer_info)
...
PrimFunc CompactBufferAllocationFeatureExtraction(PrimFunc f) {
  // Only apply this pass to TIR that is not from TE schedules
  if (!IsFromLegacyTESchedule(f)) {
    PrimFuncNode* fptr = f.CopyOnWrite();
    std::unordered_map<Buffer, Region, ObjectPtrHash, ObjectPtrEqual> region =
        BufferAccessRegionCollectorFeatureExtraction::Collect(f);
    std::unordered_map<Buffer, StorageAlignAnnotation, ObjectPtrHash, ObjectPtrEqual>
        storage_align = StorageAlignCollectorFeatureExtraction::Collect(f);
    fptr->body = BufferCompactorFeatureExtraction::Compact(f, region, storage_align);
    return f;
  } else {
    return f;
  }
...
Pass CompactBufferAllocationFeatureExtraction() {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    return CompactBufferAllocationFeatureExtraction(std::move(f));
  };
  return CreatePrimFuncPass(pass_func, 0, "tir.CompactBufferAllocationFeatureExtraction", {});
}

TVM_REGISTER_GLOBAL("tir.transform.CompactBufferAllocationFeatureExtraction")
    .set_body_typed(CompactBufferAllocationFeatureExtraction);
}  // namespace transform
