...
class Reduction(ScheduleRule):
    ...
    def apply(  # pylint: disable=too-many-locals,too-many-branches,too-many-return-statements
        ...
        if is_inner_reduction:
            self._sch_inner_reduction(sch, target, block, c_factor, epilogue, loop_order, s_split_index)
        else:
            self._sch_inner_spatial(sch, target, block, block_info, c_factor, epilogue, loop_order, s_split_index)
        return sch
        ...
        for i in range(len(s_block_var_loops)):
            for j in range(len(s_loops)):
                if s_block_var_loops[i] == s_loops[j]:
                    loop_order[i] = j
                    break
                elif s_block_var_loops[i] == s_split_loop:
